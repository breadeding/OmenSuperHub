  ...
  public partial class HelpForm : Form {
    ...
    public HelpForm() {
      ...
      var richTextBox = new RichTextBox() {
        ...
        Text = "版本号：" + version +
         "\n更新说明：\n" +
         "（1）功能：根据机型自适应添加OGH所支持的模式（Eco等），点击狂暴/极限/大师模式后恢复Tpp设定；\n" +
         "（2）功能：将热切换选项独立于冷切图形模式，并在可用时显示；\n" +
         "（3）修复：仅核显模式查询独显导致的错误，无NVIDIA独显时关闭GPU监控、频率限制和DB版本选项；\n" +
         "（4）修复：混合图形模式在不同机型上的设定不同导致的错误；\n" +
         "（5）修复：使用浮窗显示可能出现的错误。\n\n" +
         ...
         "（3）选择固定转速时，可能出现转速过低又忘记调整而导致温度过高的情况，因此OSH将在CPU温度高于（平台最高温度-5）℃时弹出通知警告并转为自动风扇模式，只在固定转速低于90%最大转速时触发，只在能够识别到最大转速的机型上触发；\n" +
