    ...
    public HelpForm() {
      ...
      var richTextBox = new RichTextBox() {
        ...
        Text = "版本号：" + version +
         ...
         "（3）选择固定转速时，可能出现转速过低又忘记调整而导致温度过高的情况，因此OSH将在CPU温度高于90摄氏度时弹出通知警告并转为自动风扇模式；\n" +
         ...
         "（2）显卡功耗=BTGP（基础功耗）+CTGP（可配置功耗）+DB（动态提升功耗），DB的含义是在CPU功率较低时额外提升GPU功耗，以暗9-i9-4060为例，BTGP=80W，BTGP+CTGP=115W，默认整机功耗为170W，显卡最大功耗140W，因此CPU功耗由60W降至30W时（具体阈值取决于Tpp），显卡总功耗将由110W升至140W，即DB功耗由-5W升至25W。在默认状态下，开启CTGP和DB才能获得最大GPU性能；\n" +
         ...
         "（4）点击“解锁版本”，程序会删除解锁版本之外的DB驱动，只使用537.42对应DB驱动，然后自动启用再禁用驱动完成解锁，这会让显卡锁定当前功耗状态（必须为40系及以下显卡），即BTGP、CTGP和DB，利用这一点可以绕开CPU功率较高时DB功率降低的限制。点击“普通版本”会重新启用上述驱动，GPU功耗将取消锁定。注意更新NVIDIA显卡驱动后会更新DB驱动，需要重新解锁；\n" +
         ...
         "（7）修改PL4会影响CPU最大功耗和整机功耗，且不一定完全等同于功耗；\n" +
         "（8）修改CPU功率会同时修改PL1与PL2，点击一次只设定一次，因此同时使用ThrottleStop控制会导致设置被覆盖；\n" +
         "（9）修改GPU频率限制能实现限制不同级别的功耗，效果相当于小飞机中拉平曲线，注意该功能不是超频功能。\n\n" +
