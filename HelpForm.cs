...
<h2>一. “风扇配置”菜单说明</h2>
<p>(1) 本程序支持安静、降温和自定义三种温度-转速配置。安静模式加载 ""silent.txt""，降温模式加载 ""cool.txt""，自定义模式可在曲线编辑器中调整 CPU/GPU 曲线并保存为 ""custom.txt""；</p>
<p>(2) 自定义曲线中左键增加节点、拖动调整节点、右键删除节点，“加载”可将其他 txt 配置载入编辑器；程序会自动进行线性插值，精度为1℃；</p>
...
<h2>一. 「風扇配置」選單說明</h2>
<p>(1) 本程式支援安靜、降溫和自訂三種溫度-轉速設定。安靜模式載入 ""silent.txt""，降溫模式載入 ""cool.txt""，自訂模式可在曲線編輯器中調整 CPU/GPU 曲線並儲存為 ""custom.txt""；</p>
<p>(2) 自訂曲線中左鍵新增節點、拖曳調整節點、右鍵刪除節點，「載入」可將其他 txt 設定載入編輯器；程式會自動進行線性插值，精度為1℃；</p>
...
<p>(1) Three fan profiles are supported. Silent loads ""silent.txt"", Cool loads ""cool.txt"", and Custom opens a CPU/GPU curve editor that saves to ""custom.txt"";</p>
<p>(2) In the custom editor, left-click to add a point, drag to adjust it, and right-click to delete it. Load imports another txt profile. Curves use linear interpolation at 1°C precision;</p>
