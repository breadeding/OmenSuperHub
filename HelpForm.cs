...
namespace OmenSuperHub {
  public partial class HelpForm : Form {
    ...
    private static string GetHelpText_ZH(Version version) {
      return "版本号：" + version +
        "\n更新说明：\n" +
        "（1）新增：浮窗显示功能支持多显示器，新增显示器选择菜单；\n" +
        "（2）新增：风扇配置菜单添加高温自动保护选项，控制是否在温度过高时取消固定转速；\n" +
        "（3）新增：根据机型自动识别是否支持CPU功率设置；\n" +
        "（4）新增：将硬件监控设置纳入自定义预设控制范围；\n" +
        "（5）修复：切换语言后菜单状态重置为默认值的问题；\n" +
        "（6）修复：本机信息中显卡列表可能识别到虚拟显示器的问题。\n\n" +
